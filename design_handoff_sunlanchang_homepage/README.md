...
## Overview
Personal homepage for **Lanchang Sun** (sunlanchang.com) — "quiet confidence" direction: warm white + charcoal, large restrained typography, matching dark bands for the featured projects (Ciyo, then Valuery). One page: hero → Ciyo → Valuery → other experiments → notes + about → footer.
...
- Accents (status dots, one per product, same chroma/lightness): Ciyo `oklch(0.72 0.13 250)`, Valuery `oklch(0.72 0.13 60)`, Buzi `oklch(0.72 0.13 320)`. Link accent on light bg: `oklch(0.55 0.09 250)`.
...
3. **Ciyo dark band** (full-bleed `#1d1b18`, inner 1180px, 88px vertical padding) — header row: 8px blue status dot + "Ciyo" 44px/600 + "BUILDING NOW" uppercase 13px letter-spacing .1em at 45% opacity; right: "ciyo.ai →" link in blue accent. One-liner 19px max-width 600px. Live website preview: sandboxed, lazy-loaded iframe of `https://ciyo.ai`, 1440×900 desktop viewport scaled to fit a 16/10 panel. Radius 14px, 1px light border, toolbar with “ciyo.ai · live preview” and an “Open website ↗” link opening a new tab. Below: three columns (flex, gap 56px, max-width 240px each) — Hypothesis / Exists today / Testing, uppercase 12px labels + 14px body.
4. **Valuery dark band** — directly below Ciyo, using the same layout and live preview component, with an orange accent and a light top border. Status: “In progress”. Description: “A value-investing iPhone app in development, built around business quality, fair-value estimates, and a margin of safety.” Links to `https://valuery.ai`; the 1440×900 iframe scales to a 16/10 panel. No facts row.
5. **Other experiments** — h3 26px/600, single-column grid gap 20px. One project: Buzi (the AI SEO assistant). Card: `#fdfcfa`, 1px border, radius 14px, padding 26px; status dot + label 12px, name 18px/600, desc 14px/1.5. Buzi links to `https://sunlanchang.com/buzi` (hover: border darkens).
6. **Notes + About** — 2-col grid gap 56px. Notes: h3 26px, hairline-separated rows (14px vertical padding): title 16px link left, date 13px tertiary right. About: h3 26px + paragraph 16px/1.65 at 65% opacity (exact copy in reference, ends "You can call me Chris.").
7. **Footer** — hairline top; "© 2026 Lanchang Sun" left; right: `sunlanchang@outlook.com` (mailto) and `@chris___sun` → `https://x.com/chris___sun`, 13px at 50–60% opacity.
...
## State Management
Reveal observer and ResizeObserver for fitting each live preview to its container. All content static/prerendered. Notes list ← MDX frontmatter (title, date) when wired.

## Assets
- No images shipped. Ciyo and Valuery render their real websites in iframes; both destinations must permit embedding.
- Status dots are pure CSS; no icon set required.
