...
- Accents (status dots, one per product, same chroma/lightness): Ciyo `oklch(0.72 0.13 250)`, Options `oklch(0.72 0.13 60)`, Buzi `oklch(0.72 0.13 320)`. Link accent on light bg: `oklch(0.55 0.09 250)`.
...
4. **Other experiments** — h3 26px/600, 2-col grid gap 20px. Two projects: Buzi (the AI SEO assistant) and Valuery (the options premium explorer). Cards: `#fdfcfa`, 1px border, radius 14px, padding 26px; status dot + label 12px, name 18px/600, desc 14px/1.5. Buzi links to `https://sunlanchang.com/buzi`; Valuery links to `https://valuery.ai` (hover: border darkens).
...
## State Management
Reveal observer and ResizeObserver for fitting the Ciyo preview to its container. All content static/prerendered. Notes list ← MDX frontmatter (title, date) when wired.

## Assets
- No images shipped. The Ciyo preview renders the real website in an iframe; the destination must permit embedding.
